class UserStocksController < ApplicationController
  ...
  def create
    ...
    if params[:stock_id].present?  # Si la accion ya estaba en la DB propia
      @user_stock = UserStock.new(stock_id: params[:stock_id], user: current_user)
    else
      stock = Stock.find_by_ticker(params[:stock_ticker]) # Busca la accion por ticker en la propia DB
      if stock
        @user_stock = UserStock.new(user: current_user, stock: stock)
      else
        stock = Stock.new_from_lookup(params[:stock_ticker]) # Si no le encuentra en la propia DB ahora le busca en la de la gema
        if stock.save # Si la encuentra en la gema y la guarda
        @user_stock = UserStock.new(user: current_user, stock: stock)
        else
          @user_stock = nil
